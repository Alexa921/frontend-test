 ...
 respuestaTestError: any = {}
  ...
  Guardar(mockResponse?: any) {
    return new Promise((resolve) => {
        ...
        if (!this.password) {
            this.respuestaTest = { state: false, mensaje: "El password es requerido" };
            resolve(false);
            return;
        }
        ...
        const response = mockResponse || this.peticion.Post(post.host + post.path, post.payload);
        
        response.then((res: { state: boolean; mensaje: string }) => {
  ...
  ListarTodos(){
    var post = {
      ...
      this.peticion.Get(this.peticion.urlHost + "/usuarios/listarTodos")
      .then((res: any) => {
